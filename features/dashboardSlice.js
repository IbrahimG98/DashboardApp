...
export const retrieveDashboard = createAsyncThunk(
  ...
  async () => {
    try {
      ...
      console.log("retrieve dash", dataJson);
...
export const retrieveAvailableSurveys = createAsyncThunk(
  ...
  async () => {
    try {
      ...
      const dataJson = await res.json();
      console.log("retrieve available surveys", dataJson);
  ...
  extraReducers: (builder) => {
    builder.addCase(loginDashboard.fulfilled, (state, action) => {
      if (action?.payload?.requestSuccessfull) {
        console.log("LOGIN DASHBOARD", action.payload.data);
        return {
          ...state,
          user: {
            ...state.user,
...
export const selectUserToken = (state) => state.dashboard.user.token;
export const selectUserRefreshToken = (state) =>
  state.dashboard.user.refreshToken;
